...
```
dist/
├── codemind-v0.4.0.tar.gz          # 部署包
└── codemind-v0.4.0.tar.gz.sha256   # 校验文件
```
...
```bash
# 从本地上传（在开发机器上执行）
scp dist/codemind-v0.4.0.tar.gz user@your-server-ip:/tmp/
```
...
codemind-backend    codemind-backend:0.4.0        Up (healthy)
codemind-frontend   codemind-frontend:0.4.0       Up           0.0.0.0:18080->80/tcp
