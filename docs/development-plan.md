...
公司内部需要为员工提供 AI 编码辅助服务，基于本地服务器部署的 LLM 编码模型。为了有效管理用户、控制资源使用、保障服务质量，需要开发一个上层管理平台，类似于 Kimi、GLM 等公司提供的开发者控制台。

### 1.2 项目目标

- 提供统一的 AI 编码服务入口，管理用户访问权限
- 实现多层级用户管理体系（超级管理员 / 部门经理 / 普通用户）
- 代理转发所有 LLM 请求，实现 Token 用量统计与限额控制
- 提供 API Key 管理机制，支持用户通过 Key 接入服务
- 提供可视化的用量统计面板
...
zhangsan,张三,zhangsan@company.com,13800138000,1,user
lisi,李四,lisi@company.com,13700137000,2,user
```
