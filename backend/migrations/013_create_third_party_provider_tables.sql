...
CREATE TABLE IF NOT EXISTS third_party_provider_templates (
...
CREATE INDEX IF NOT EXISTS idx_tppt_status ON third_party_provider_templates(status) WHERE deleted_at IS NULL;

-- 用户第三方模型服务（用户独立绑定）
CREATE TABLE IF NOT EXISTS user_third_party_providers (
    id                 BIGSERIAL PRIMARY KEY,
...
CREATE INDEX IF NOT EXISTS idx_utpp_user_id ON user_third_party_providers(user_id) WHERE deleted_at IS NULL;
CREATE UNIQUE INDEX IF NOT EXISTS idx_utpp_user_name ON user_third_party_providers(user_id, name) WHERE deleted_at IS NULL;

-- 第三方模型服务用量记录（与平台用量分开统计）
CREATE TABLE IF NOT EXISTS third_party_token_usage (
    id                BIGSERIAL PRIMARY KEY,
...
CREATE INDEX IF NOT EXISTS idx_tptu_user_created ON third_party_token_usage(user_id, created_at);
CREATE INDEX IF NOT EXISTS idx_tptu_provider ON third_party_token_usage(provider_id);
