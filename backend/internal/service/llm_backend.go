...
// LLMBackendService LLM 后端节点管理业务逻辑
...
// NewLLMBackendService 创建 LLM 后端管理服务
...
// RefreshLoadBalancer 从数据库加载后端列表并刷新负载均衡器
func (s *LLMBackendService) RefreshLoadBalancer() {
	s.refreshLoadBalancer()
}

// refreshLoadBalancer 内部方法：刷新负载均衡器节点
func (s *LLMBackendService) refreshLoadBalancer() {
	...
	if err != nil {
		s.logger.Error("加载 LLM 后端列表失败", zap.Error(err))
		return
	}
