...
func getOverviewContent() string {
	return `## 欢迎使用 CodeMind 接入指南

CodeMind 是企业级 AI 编码服务平台，兼容 **OpenAI 协议** 和 **Anthropic 协议**，可接入各种主流 AI 编码工具。
...
| 协议类型 | 接入地址（Base URL） |
|---------|-------------------|
| OpenAI 兼容协议 | ` + "`http://192.168.0.27:18080/api/openai/v1`" + ` |
| Anthropic 协议 | ` + "`http://192.168.0.27:18080/api/anthropic`" + ` |

> 大多数工具使用 **OpenAI 兼容协议**即可。部分工具（如 Claude Code、Factory Droid、Goose）支持 Anthropic 协议，可获得更好的兼容性。

## 可用模型

| 模型名称 | 说明 |
|---------|------|
| ` + "`qwen3.5-35b-a3b`" + ` | 千问 3.5 模型，推荐使用 |

> 不同工具中模型名称的大小写要求可能不同，请参考对应工具的接入文档。
...
1. **获取 API Key** — 在平台「API Keys」页面创建
2. **配置工具** — 在您使用的 AI 编码工具中填入 Base URL 和 API Key
3. **选择模型** — 选择 ` + "`qwen3.5-35b-a3b`" + ` 或其他可用模型
4. **开始使用** — 配置完成后即可开始 AI 辅助编码

## 支持的工具

请从左侧菜单选择您使用的工具，查看详细的配置指南：

- **Claude Code** — 终端 AI 编码工具
- **Claude Code IDE 插件** — VS Code / JetBrains 集成
- **Cursor** — AI 驱动的代码编辑器
- **TRAE** — 智能 IDE
- **Cline / Kilo Code / Roo Code** — VS Code AI 插件
- **更多工具** — 支持 OpenAI 协议的其他工具

## 常见问题

### 连接失败？

- 确认 Base URL 填写正确（注意末尾不要有多余的 ` + "`/`" + `）
- 检查 API Key 是否有效
- 确认网络可以访问平台地址
...
- 确认模型名称拼写正确
- 部分工具需要使用大写模型名称（如 ` + "`QWEN3.5-35B-A3B`" + `）
- 检查您的账户是否具有该模型的使用权限
`
}
...
运行以下命令配置 CodeMind：
	...
	claude config set anthropic_base_url http://192.168.0.27:18080/api/anthropic/v1
...
4. 在 **Override OpenAI Base URL** 中，将默认 URL 替换为：http://192.168.0.27:18080/api/openai/v1
5. 输入您希望使用的模型，如 QWEN3.5-35B-A3B

![Cursor OpenAI配置](/images/docs/cursor-openai-config.jpeg)

> **注意**：在 Cursor 中，需要输入模型的大写名称，如 QWEN3.5-35B-A3B

### 3. 保存并切换模型

配置完成后，保存设置并在主页上选择您刚创建的 QWEN3.5-35B-A3B Provider。
...
- **Base URL**: http://192.168.0.27:18080/api/openai/v1
- **API Key**: 从 CodeMind 平台获取的 API Key
- **Model**: 选择 QWEN3.5-35B-A3B
...
配置完成后，保存设置并在自定义模型中选择您刚创建的 QWEN3.5-35B-A3B。
...
- **Base URL**：输入 http://192.168.0.27:18080/api/openai/v1
- **API Key**：填入您的 CodeMind API Key
- **模型**：选择"使用自定义"，并输入模型名称（如：qwen3.5-35b-a3b）
- **其他配置**：
  - 取消勾选 Support Images
...
- **Base URL**：输入 http://192.168.0.27:18080/api/openai/v1
- **API Key**：填入您的 CodeMind API Key
- **Model**：选择 qwen3.5-35b-a3b 或者列表中您想使用的模型
...
` + "```json\n" + `{
  "customModels": [
    {
      "displayName": "QWEN3.5-35B-A3B [CodeMind]",
      "model": "qwen3.5-35b-a3b",
      "baseUrl": "http://192.168.0.27:18080/api/anthropic",
      "apiKey": "your_api_key",
      "provider": "anthropic",
...
` + "```json\n" + `{
  "customModels": [
    {
      "displayName": "QWEN3.5-35B-A3B [CodeMind] - OpenAI",
      "model": "qwen3.5-35b-a3b",
      "baseUrl": "http://192.168.0.27:18080/api/openai/v1",
      "apiKey": "your_api_key",
...
` + "```json\n" + `{
  "providers": {
    "codemind": {
      ...
      "base_url": "http://192.168.0.27:18080/api/openai/v1",
...
配置完成后，运行 crush 命令启动，选择 QWEN3.5-35B-A3B 模型进行操作。
...
2. 填写以下必要的配置：
   - **Base URL**: http://192.168.0.27:18080/api/anthropic
   - **API Key**: 您的 CodeMind API Key
   - **Model**: 选择 qwen3.5-35b-a3b
...
` + "```json\n" + `{
  "agents": {
    "defaults": {
      "model": {
        "primary": "zai/qwen3.5-35b-a3b",
        "fallbacks": ["zai/qwen3.5-35b-a3b"]
      }
    }
  }
}` + "\n```\n" + `
...
3. 变更 API 地址为：http://192.168.0.27:18080/api/openai/v1/

![Cherry Studio API配置](/images/docs/cherry-api-config.png)

4. 点击下方管理，将 qwen3.5-35b-a3b 模型添加

### 3. 开始使用

配置完成后，在对话中选择您刚配置的 QWEN3.5-35B-A3B 使用。
...
2. **添加/替换 OpenAI Base URL 为：http://192.168.0.27:18080/api/openai/v1**
...
- qwen3.5-35b-a3b: 最新最强编码模型
- qwen3.5-35b-a3b: 标准版本，适合复杂任务
- qwen3.5-35b-a3b: 轻量版本，响应更快
`
}
