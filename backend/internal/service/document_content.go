...
| 协议类型 | 接入地址（Base URL） |
|---------|-------------------|
| OpenAI 兼容协议 | ` + "`http://192.168.0.27:8080/v1`" + ` |
| Anthropic 协议 | ` + "`http://192.168.0.27:8080/v1`" + ` |
...
运行以下命令配置 CodeMind：
	...
	claude config set anthropic_base_url http://192.168.0.27:8080/v1
...
4. 在 **Override OpenAI Base URL** 中，将默认 URL 替换为：http://192.168.0.27:8080/v1
...
- **Base URL**: http://192.168.0.27:8080/v1
...
- **Base URL**：输入 http://192.168.0.27:8080/v1
...
` + "```json\n" + `{
  "providers": {
    "codemind": {
      ...
      "base_url": "http://192.168.0.27:8080/v1",
...
3. 变更 API 地址为：http://192.168.0.27:8080/v1/
...
2. **添加/替换 OpenAI Base URL 为：http://192.168.0.27:8080/v1**
