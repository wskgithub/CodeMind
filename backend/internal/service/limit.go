...
// LimitService manages quota limits with elastic hour-based periods.
...
// List returns rate limit configurations.
...
// Upsert creates or updates a rate limit configuration.
...
// Delete removes a rate limit configuration.
...
// cycleStartKey returns the Redis key for cycle start time.
func cycleStartKey(userID int64, ruleID int64) string {
	return fmt.Sprintf("codemind:cycle:%d:%d:start", userID, ruleID)
}

// cycleUsageKey returns the Redis key for current cycle usage.
func cycleUsageKey(userID int64, ruleID int64) string {
	return fmt.Sprintf("codemind:cycle:%d:%d:usage", userID, ruleID)
}

// CheckAllQuotas checks all quota rules for a user.
// Returns true if quota is sufficient, false if any rule is exceeded.
func (s *LimitService) CheckAllQuotas(ctx context.Context, userID int64, deptID *int64) (bool, error) {
	...
	if err != nil {
		s.logger.Warn("failed to get limits, allowing request", zap.Error(err))
		return true, nil
	}
	...
	for _, limit := range limits {
		...
		if err != nil {
			s.logger.Warn("failed to check limit, allowing request",
...
// isRuleExceeded checks if a single rule is exceeded.
...
// RecordCycleUsage records usage to each rule's current cycle.
func (s *LimitService) RecordCycleUsage(ctx context.Context, userID int64, deptID *int64, tokens int) {
	limits, err := s.limitRepo.GetAllEffectiveLimits(userID, deptID)
	if err != nil {
		s.logger.Error("failed to get limits for recording usage", zap.Error(err))
		return
	}
...
// recordRuleUsage records usage for a single rule.
func (s *LimitService) recordRuleUsage(ctx context.Context, userID int64, rule *model.RateLimit, tokens int64) {
	...
	if err != nil {
		s.logger.Error("failed to read cycle start time", zap.Error(err))
		return
	}
...
// GetLimitProgress returns user's quota progress including reset times.
...
// GetMyLimits returns current user's limits and usage (legacy API).
