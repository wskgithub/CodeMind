...
const DashboardPage = () => {
  ...
  // re-render chart on theme or language change
  useEffect(() => {
  ...
  const renderChart = () => {
    ...
    const legendData = hasThirdPartyData
      ? [t('dashboard.charts.platformToken'), t('dashboard.charts.thirdPartyToken'), t('dashboard.charts.requestCount')]
      : [t('dashboard.charts.tokenUsage'), t('dashboard.charts.requestCount')];

    const barSeries: echarts.SeriesOption[] = [
      {
        name: hasThirdPartyData ? t('dashboard.charts.platformToken') : t('dashboard.charts.tokenUsage'),
        type: 'bar',
        stack: 'tokens',
        data: tokens,
        itemStyle: {
    ...
    if (hasThirdPartyData) {
      barSeries.push({
        name: t('dashboard.charts.thirdPartyToken'),
        type: 'bar',
        stack: 'tokens',
        data: tpTokens,
        itemStyle: {
    ...
    chartInstance.current.setOption({
      ...
      yAxis: [
        {
          type: 'value',
          name: t('dashboard.charts.tokenUsage'),
    ...
    <div className="animate-fade-in-up">
        ...
        <Col xs={24} sm={12} lg={6}>
          <div className="stat-card animate-fade-in-up h-full" style={{ animationDelay: '0.05s' }}>
            <div className="flex items-center gap-4 h-full">
              ...
              />
              <div>
                <div style={{ color: isDark ? 'rgba(255, 255, 255, 0.5)' : 'rgba(0, 0, 0, 0.5)', fontSize: 13, marginBottom: 4 }}>{t('dashboard.stats.todayTokenUsage')}</div>
                ...
                {(overview?.today.third_party_total_tokens || 0) > 0 && (
                  <div style={{ color: isDark ? 'rgba(255, 255, 255, 0.35)' : 'rgba(0, 0, 0, 0.35)', fontSize: 11, marginTop: 2 }}>
                    {t('dashboard.platform')} {formatNumber(overview?.today.total_tokens || 0)} · {t('dashboard.thirdParty')} {formatNumber(overview?.today.third_party_total_tokens || 0)}
                  </div>
                )}
              </div>
            </div>
        ...
        <Col xs={24} sm={12} lg={6}>
          <div className="stat-card animate-fade-in-up h-full" style={{ animationDelay: '0.1s' }}>
            <div className="flex items-center gap-4 h-full">
              ...
              />
              <div>
                <div style={{ color: isDark ? 'rgba(255, 255, 255, 0.5)' : 'rgba(0, 0, 0, 0.5)', fontSize: 13, marginBottom: 4 }}>{t('dashboard.stats.todayRequests')}</div>
                <div style={{ color: isDark ? '#fff' : 'rgba(0, 0, 0, 0.85)', fontSize: 24, fontWeight: 700 }}>
                ...
                {(overview?.today.third_party_total_requests || 0) > 0 && (
                  <div style={{ color: isDark ? 'rgba(255, 255, 255, 0.35)' : 'rgba(0, 0, 0, 0.35)', fontSize: 11, marginTop: 2 }}>
                    {t('dashboard.platform')} {(overview?.today.total_requests || 0).toLocaleString()} · {t('dashboard.thirdParty')} {(overview?.today.third_party_total_requests || 0).toLocaleString()}
                  </div>
                )}
              </div>
            </div>
            ...
            <Col xs={24} sm={12} lg={6}>
              <div className="stat-card animate-fade-in-up h-full" style={{ animationDelay: '0.15s' }}>
                <div className="flex items-center gap-4 h-full">
                  ...
                  />
                  <Statistic
                    title={<span style={{ color: isDark ? 'rgba(255, 255, 255, 0.5)' : 'rgba(0, 0, 0, 0.5)', fontSize: 13 }}>{t('dashboard.stats.totalUsers')}</span>}
            ...
            <Col xs={24} sm={12} lg={6}>
              <div className="stat-card animate-fade-in-up h-full" style={{ animationDelay: '0.2s' }}>
                <div className="flex items-center gap-4 h-full">
                  ...
                  />
                  <Statistic
                    title={<span style={{ color: isDark ? 'rgba(255, 255, 255, 0.5)' : 'rgba(0, 0, 0, 0.5)', fontSize: 13 }}>{t('dashboard.stats.activeApiKeys')}</span>}
            ...
            <Col xs={24} sm={12} lg={6}>
              <div className="stat-card animate-fade-in-up h-full" style={{ animationDelay: '0.15s' }}>
                <div className="flex items-center gap-4 h-full">
                  ...
                  />
                  <div>
                    <div style={{ color: isDark ? 'rgba(255, 255, 255, 0.5)' : 'rgba(0, 0, 0, 0.5)', fontSize: 13, marginBottom: 4 }}>{t('dashboard.stats.monthlyTokenUsage')}</div>
                    <div style={{ color: isDark ? '#fff' : 'rgba(0, 0, 0, 0.85)', fontSize: 24, fontWeight: 700 }}>
                    ...
                    {(overview?.this_month.third_party_total_tokens || 0) > 0 && (
                      <div style={{ color: isDark ? 'rgba(255, 255, 255, 0.35)' : 'rgba(0, 0, 0, 0.35)', fontSize: 11, marginTop: 2 }}>
                        {t('dashboard.platform')} {formatNumber(overview?.this_month.total_tokens || 0)} · {t('dashboard.thirdParty')} {formatNumber(overview?.this_month.third_party_total_tokens || 0)}
                      </div>
                    )}
                  </div>
                </div>
            ...
            <Col xs={24} sm={12} lg={6}>
              <div className="stat-card animate-fade-in-up h-full" style={{ animationDelay: '0.2s' }}>
                <div className="flex items-center gap-4 h-full">
                  ...
                  />
                  <div>
                    <div style={{ color: isDark ? 'rgba(255, 255, 255, 0.5)' : 'rgba(0, 0, 0, 0.5)', fontSize: 13, marginBottom: 4 }}>{t('dashboard.stats.monthlyRequests')}</div>
                    <div style={{ color: isDark ? '#fff' : 'rgba(0, 0, 0, 0.85)', fontSize: 24, fontWeight: 700 }}>
                    ...
                    {(overview?.this_month.third_party_total_requests || 0) > 0 && (
                      <div style={{ color: isDark ? 'rgba(255, 255, 255, 0.35)' : 'rgba(0, 0, 0, 0.35)', fontSize: 11, marginTop: 2 }}>
                        {t('dashboard.platform')} {(overview?.this_month.total_requests || 0).toLocaleString()} · {t('dashboard.thirdParty')} {(overview?.this_month.third_party_total_requests || 0).toLocaleString()}
                      </div>
                    )}
                  </div>
                </div>
      ...
      >
        <h3 style={{ 
          ...
          {t('dashboard.charts.last30DaysTrend')}
        </h3>
        {usageData.length > 0 ? (
          <div
            ref={chartRef}
          ...
          <Empty
            ...
            description={<span style={{ color: isDark ? 'rgba(255, 255, 255, 0.5)' : 'rgba(0, 0, 0, 0.5)' }}>{t('dashboard.charts.noData')}</span>}
            style={{ padding: 40 }}
          />
        )}
      </div>
            ...
            {announcements.slice(0, 5).map((ann, index) => (
              ...
              >
                <div className="flex items-center gap-2 flex-wrap">
                  {ann.pinned && (
                    <Tag color="error" style={{ borderRadius: 4, border: 'none' }}>{t('dashboard.announcements.pinned')}</Tag>
                  )}
