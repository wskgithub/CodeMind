...
const ProviderTemplatesPage: React.FC = () => {
  ...
  const handleDelete = (record: ProviderTemplate) => {
    Modal.confirm({
      ...
      content: t('templates.confirmDeleteContent', { name: record.name }),
  ...
  const columns: ColumnsType<ProviderTemplate> = useMemo(() => [
    {
      title: t('templates.table.templateName'),
      dataIndex: 'name',
      key: 'name',
    ...
    {
      title: t('templates.table.protocol'),
      dataIndex: 'format',
      key: 'format',
      width: 160,
    ...
    {
      title: t('templates.table.description'),
    ...
    {
      title: t('templates.table.sortOrder'),
    ...
    },
    {
      ...
      render: (_, record) => (
        <Space>
          ...
          <Button type="link" size="small" danger icon={<DeleteOutlined />} onClick={() => handleDelete(record)}>{t('common.delete')}</Button>
        </Space>
      ),
    },
  ], [isDark]);
      ...
      <div className="animate-fade-in-up" style={{ position: 'relative', zIndex: 1 }}>
        ...
        <div className="glass-card animate-fade-in-up" style={{ padding: 24, animationDelay: '0.05s' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
            <span style={{ fontWeight: 600, color: isDark ? '#fff' : 'rgba(0, 0, 0, 0.85)', fontSize: 16 }}>{t('templates.templateList')}</span>
        ...
        >
          <Form form={form} layout="vertical" onFinish={handleSubmit}>
            <Form.Item name="name" label={<span style={{ color: labelColor }}>{t('templates.table.templateName')}</span>}
              rules={[{ required: true, message: t('templates.form.nameRequired') }]}>
              <Input placeholder={t('templates.form.namePlaceholder')} style={inputStyle} />
            </Form.Item>

            <Form.Item name="format" label={<span style={{ color: labelColor }}>{t('templates.form.protocolFormat')}</span>}
              rules={[{ required: true, message: t('templates.form.protocolRequired') }]}
              extra={<span style={{ color: isDark ? 'rgba(255, 255, 255, 0.35)' : 'rgba(0, 0, 0, 0.35)', fontSize: 12 }}>
                {t('templates.form.protocolExtra')}
              </span>}>
              <Select
                options={[
                  { label: t('templates.form.openaiProtocol'), value: 'openai' },
                  { label: t('templates.form.anthropicProtocol'), value: 'anthropic' },
                  { label: t('templates.form.allProtocols'), value: 'all' },
                ]}
                style={inputStyle}
              />
            </Form.Item>
                  ...
                  <>
                    {(fmt === 'openai' || fmt === 'all') && (
                      <Form.Item name="openai_base_url" label={<span style={{ color: labelColor }}>OpenAI Base URL</span>}
                        rules={[{ required: true, message: t('templates.form.openaiUrlRequired') }]}>
                    ...
                    {(fmt === 'anthropic' || fmt === 'all') && (
                      <Form.Item name="anthropic_base_url" label={<span style={{ color: labelColor }}>Anthropic Base URL</span>}
                        rules={[{ required: true, message: t('templates.form.anthropicUrlRequired') }]}>
            ...
            <Form.Item name="models" label={<span style={{ color: labelColor }}>{t('templates.table.modelList')}</span>}
              rules={[{ required: true, message: t('templates.form.modelsRequired') }]}
              extra={<span style={{ color: isDark ? 'rgba(255, 255, 255, 0.35)' : 'rgba(0, 0, 0, 0.35)', fontSize: 12 }}>
                {t('templates.form.modelsExtra')}
              </span>}>
              <Select mode="tags" placeholder={t('templates.form.modelsPlaceholder')} style={inputStyle} tokenSeparators={[',']} />
            </Form.Item>

            <Form.Item name="description" label={<span style={{ color: labelColor }}>{t('templates.form.descriptionOptional')}</span>}>
              <Input placeholder={t('templates.form.descriptionPlaceholder')} style={inputStyle} />
            </Form.Item>

            <Form.Item name="sort_order" label={<span style={{ color: labelColor }}>{t('templates.form.sortWeight')}</span>}>
              <InputNumber min={0} style={{ ...inputStyle, width: '100%' }} placeholder={t('templates.form.sortPlaceholder')} />
            </Form.Item>
